        ...
        if (products && products.length)
            return products.map((product, index) =>
                ...
                <li key={index} class=" rounded-lg outline outline-offset-1 outline-blue-200 outline outline-offset-4 ">
                    <h6 class="text-2xl text-indigo-500 pb-1 pt-1 not-italic font-bold font-mono">{(product) ? product.name : '-'}</h6>
                    ...
                    <h6 class="font-bold font-mono">Number:{(product) ? product.number : 0}</h6>
                    <h6 class="font-bold font-mono">price:{(product) ? product.price : 0}</h6>
            ...
            return <h1 class="font-bold font-mono text-2xl text-pink-500 items-center "> No product</h1>
    }
    return (<div class="bg-red-100 sm:h-screen items-center " >
        <Navbar />
        <div class="flex flex-col justify-around  items-center">
        <h1 class="text-4xl text-pink-500 pb-8 pt-8 font-bold font-mono">products Show</h1>
        <ul class=" grid grid-cols-6 gap-10  " >{printproducts()}</ul>
        </div>
    </div>
    )
